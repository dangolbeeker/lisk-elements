...
describe('api resource module', () => {
	...
	beforeEach(() => {
		...
		banActiveNodeAndSelectStub = sandbox.stub(LiskAPI, 'banActiveNodeAndSelect');
	...
	describe('#constructor', () => {
		...
		it('should throw error without input', () => {
			return (() => new APIResource()).should.throw('Require LiskAPI instance to be initialized.');
		});
	});
	...
	describe('#getResourcePath', () => {
		...
		it('should return resource full path with set path', () => {
			...
			return resource.resourcePath.should.equal(`${defaultBasePath}/api${defaultResourcePath}`);
		});
	});
	...
	describe('#handleRetry', () => {
		let requestStub;
		beforeEach(() => {
			requestStub = sandbox.stub(resource, 'request').returns(Promise.resolve(sendRequestResult.body));
		});
		...
		describe('when there is no available node', () => {
			...
			it('should resolve with failure response', () => {
				...
				req.then(res => {
					...
					return res.message.should.equal('Could not create an HTTP request to any known nodes.');
				});
			});
		});
	});
